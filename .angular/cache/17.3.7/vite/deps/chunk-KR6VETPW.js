...
function fromRef(ref2, event) {
  return new Observable(function(subscriber) {
    var fn = ListenerMethods[event](ref2, function(snapshot, prevKey) {
...
function object(query2) {
  return fromRef(query2, ListenEvent.value);
}
function objectVal(query2, options) {
  if (options === void 0) {
    options = {};
  }
  return fromRef(query2, ListenEvent.value).pipe(map(function(change) {
...
function stateChanges(query2, options) {
  if (options === void 0) {
    options = {};
  }
  var events = validateEventsArray(options.events);
  var childEvent$ = events.map(function(event) {
    return fromRef(query2, event);
  });
  return merge.apply(void 0, childEvent$);
}
function get2(query2) {
  return from(get(query2)).pipe(map(function(snapshot) {
    var event = ListenEvent.value;
    return { snapshot, prevKey: null, event };
  }));
}
function list(query2, options) {
  if (options === void 0) {
    options = {};
  }
  var events = validateEventsArray(options.events);
  return get2(query2).pipe(switchMap(function(change) {
    var childEvent$ = [of(change)];
    events.forEach(function(event) {
      childEvent$.push(fromRef(query2, event));
    });
    return merge.apply(void 0, childEvent$).pipe(scan(buildView, []));
  }), distinctUntilChanged());
}
function listVal(query2, options) {
  if (options === void 0) {
    options = {};
  }
  return list(query2).pipe(map(function(arr) {
...
function auditTrail(query2, options) {
  ...
  var auditTrail$ = stateChanges(query2, options).pipe(scan(function(current, changes) {
    return __spreadArray(__spreadArray([], current, true), [changes], false);
  }, []));
  return waitForLoaded(query2, auditTrail$);
}
function loadedData(query2) {
  return fromRef(query2, ListenEvent.value).pipe(map(function(data) {
    var lastKeyToLoad;
    data.snapshot.forEach(function(child2) {
...
function waitForLoaded(query2, snap$) {
  var loaded$ = loadedData(query2);
  return loaded$.pipe(
    withLatestFrom(snap$),
...
rxfire/database/index.esm.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-KR6VETPW.js.map
